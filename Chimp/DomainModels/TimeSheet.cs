...
public record TimeSheet(TimeSheetRow[] Rows, ProjectTask[] ProjectTasks, Tag[] Tags, DateOnly Date)
{
    ...
    public TimeSheetRow GetRow(ShortId<TimeSheetRow> shortId)
        => Rows.SingleOrDefault(r => r.ShortId == shortId) ?? throw new Error("previously fetched timesheet does not contain line #{Line}", shortId);

    public ProjectTask GetProjectTask(ShortId<ProjectTask> shortId)
        => ProjectTasks.SingleOrDefault(p => p.ShortId == shortId) ?? throw new Error("previously fetched projects list does not contain line #{Line}", shortId);

    public Tag GetTag(ShortId<Tag> shortId)
        => Tags.SingleOrDefault(t => t.ShortId == shortId) ?? throw new Error("previously fetched tags list does not contain tag {Tag}", "#" + shortId);
}
